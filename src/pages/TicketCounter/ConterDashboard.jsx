...
const CounterDashboard = () => {
  ...
  const user = useSelector((state) => state.auth.user || 'Admin'); // Fallback for user
  ...
  // Static notification data (could be dynamic if API provides it)
  ...
  // Format stats data with null safety
  ...
  // Format recent bookings data with null checks
  ...
  // Format active trips data with route lookup
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {activeTrips.map((trip) => (
                    <tr key={trip.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-500">{trip.route}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{trip.departure}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{trip.arrival}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{trip.bus}</td>
                      <td className="px-4 py-3 text-sm">
                        <span
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
